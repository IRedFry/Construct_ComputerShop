...
namespace BLL
{
    public class DBDataOperation : IDbCrud
    {
        ...
        public void CreateProduct(ProductModel p)
        {
            ...
            context.Products.Create(new Product {id = id, price = p.Price, description = p.Description, name = p.Name, id_category = p.CategoryId, id_manufacturer = p.ManufId, sale = p.Sale, count = 1, Manufacturer = m, Category = c});
            Save();
        }
        ...
        public int CreateBusket()
        {
            int id = context.Orders.GetList().OrderByDescending(i => i.id).FirstOrDefault().id + 1;
            OrderC or = new OrderC
            {
                Customer = context.Customers.GetItem(0),
                date = DateTime.Now,
                id = id,
                id_client = context.Customers.GetItem(0).id,
                id_seller = context.Sellers.GetItem(0).id,
                Seller = context.Sellers.GetItem(0),
                id_status = 3,
                OrderStatus = context.Statuses.GetItem(3),
                Order_line = new List<Order_line>(),
                time = null,
                total_cost = 0
            };

            context.Orders.Create(or);
            return id;
        }
